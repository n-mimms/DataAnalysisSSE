...
library(scales) # for fixing 5E+03 etc to 5000
library(expss) # for mean_if etc (graphs)
library(tidyr) # for slope graphs
...
(Please note; In all graphs the sales figures have been divided by 100 to express them in kr and cents.)

```{r}
# other_features <- c("trueGender","month")
...
ggplot(df_final,aes(x=month))+
  geom_bar(stat="identity",aes(y=TotalSales/100), fill=ctot, alpha=0.5, width=.5) +
  geom_bar(stat="identity",aes(y=AvgSales/100), fill=cavg, alpha=.5, width=.5) +
  labs(title= "Total and Average Sales by month of first purchase") +
    scale_y_continuous(name="Total/Average sales", labels = comma)

```
(Blue is the average sales  per month of first purchase, and green is the total sales
per month of first purchase. Both are divided by 100 to be 100,50 rather than 10 050)


```{r}

fem_sales <- subset(df_final, df_final$trueGender=="female")
mal_sales <- subset(df_final,df_final$trueGender=="male")

ggplot(df_final, aes(month)) +
  geom_point(aes(month, AvgSales/100), data=fem_sales, colour="#F39C12") +
  geom_point(aes(month,AvgSales/100),data=mal_sales, colour="#7D3C98", alpha=.2)+
  labs(title="Average sales by month of first purchase" ,
  subtitle= "split by gender",
       x= "Month") +
  scale_y_continuous(name="Average sales", labels = comma)

## Can obviously also add a TotalSales (I did that first but I think AvgSales makes more sense)
## I didn't wanna mess with df_final, but there would def be a point to having dates instead? 

```
(This is the same average sales as in the last graph, but for each customer and split
by gender. Orange is female customers and purple is male customers.)


```{r}
avg_byear <- c(1:99)
for(i in 1:99) {
  avg_byear[i] <- mean_if(df_final$trueBirthyear== i, df_final$AvgSales)
}

tot_byear <- c(1:99)
for(i in 1:99) {
  tot_byear[i] <- mean_if(df_final$trueBirthyear== i, df_final$TotalSales)
}
byear <- na.omit(data.frame(c(1:99),avg_byear, tot_byear))
colnames(byear) <- c("Year","Average","Total")

ggplot(byear,aes(x=Year))+
  geom_point(aes(y=Average),col=cavg) +
  geom_smooth(aes(y=Average), method=loess, col=cavg, alpha=.5) +
  geom_point(aes(y=Total),col=ctot) +
  geom_smooth(aes(y=Total), method=loess,col=ctot, alpha=.5) +
  labs(title="Mean Average/Total Sales by birthyear", x= "Birthyear(19XX)", y="Sales")
    scale_y_continuous(name="Sales", labels = comma)
    
```
(This graph plots the average (blue) and total (green) sales for customers per 
birthyear. It also includes a trend line with standard deviation.)


- Scatterplots with features vs target, to look at the expected impact (I don't have any good ideas what to do with the target//Ylva)
...
glm.train <- confusionMatrix(pred, df_train$target_customer)
glm.test <- confusionMatrix(predict(glm.model, df_test), df_test$target_customer)
...
lda.train <- confusionMatrix(predict(lda.model, df_train), df_train$target_customer, positive = "is_target")
lda.test <- confusionMatrix(predict(lda.model, df_test), df_test$target_customer, positive = "is_target")
...
qda.train <- confusionMatrix(predict(qda.model, df_train), df_train$target_customer, positive = "is_target")
qda.test <- confusionMatrix(predict(qda.model, df_test), df_test$target_customer, positive = "is_target")
...
rf.pred <- predict(rf.model,df_test)
...
xgb.pred <- predict(modelxgboost,df_test)
...
Table based on prediction on test data.
```{r}
acc <- cbind(glm.test$overall[1],lda.test$overall[1],qda.test$overall[1],
             rf.cm.test$overall[1],xgb.cm.test$overall[1])
kappa <- cbind(glm.test$overall[2],lda.test$overall[2],qda.test$overall[2],
             rf.cm.test$overall[2],xgb.cm.test$overall[2])
sens <- cbind(glm.test$byClass[1],lda.test$byClass[1],qda.test$byClass[1],rf.cm.test$byClass[1],xgb.cm.test$byClass[1])
spec <- cbind(glm.test$byClass[2],lda.test$byClass[2],qda.test$byClass[2],rf.cm.test$byClass[2],xgb.cm.test$byClass[2])


model_summary <- rbind(acc,kappa,sens,spec)
colnames(model_summary) <- c("Logit", "LDA", "QDA", "R. Forest", "XgBoost")
print(model_summary)
```

```{r}
acc.train <- rbind(glm.train$overall[1],lda.train$overall[1],
                   qda.train$overall[1],rf.cm.train$overall[1],
                   xgb.cm.train$overall[1])
kapp.train <- rbind(glm.train$overall[2],lda.train$overall[2],
                   qda.train$overall[2],rf.cm.train$overall[2],
                   xgb.cm.train$overall[2])
acc.test <- rbind(glm.test$overall[1],lda.test$overall[1],qda.test$overall[1],rf.cm.test$overall[1],xgb.cm.test$overall[1])
kapp.test <- rbind(glm.test$overall[2],lda.test$overall[2],qda.test$overall[2],rf.cm.test$overall[2],xgb.cm.test$overall[2])

slope_acc <- data.frame(model=c("Logit","LDA","QDA","R. Forest","XgBoost"), train=acc.train, test=acc.test)
colnames(slope_acc) <- c("model","test","train")
slope_acc <- slope_acc %>% gather(2:3,key=dataset,value=accuracy)

ggplot(slope_acc,aes(x=dataset,y=accuracy,group=model)) +
  geom_line() +
  geom_point() +
  scale_x_discrete(limits=c("train","test")) +
  coord_cartesian(ylim=c(.74,.91)) +
  annotate(geom = "text", x="test", y=acc.train[1], label="Logit",hjust=-.2) +
  annotate(geom = "text", x="test", y=acc.train[2], label="LDA",hjust=-.2) +
  annotate(geom = "text", x="test", y=acc.train[3], label="QDA",hjust=-.2) +
  annotate(geom = "text", x="train", y=acc.train[4], label="R. Forest",hjust=-.2) +
  annotate(geom = "text", x="test", y=acc.train[5], label="XgBoost",hjust=-.2)
  
```
```{r}
slope_kapp <- data.frame(model=c("Logit","LDA","QDA","R. Forest","XgBoost"), train=kapp.train, test=kapp.test)
colnames(slope_kapp) <- c("model","test","train")
slope_kapp <- slope_kapp %>% gather(2:3,key=dataset,value=accuracy)

ggplot(slope_kapp,aes(x=dataset,y=accuracy,group=model)) +
  geom_line() +
  geom_point() +
  scale_x_discrete(limits=c("train","test")) +
  coord_cartesian(ylim=c(.36,.77)) +
  annotate(geom = "text", x="test", y=kapp.train[1], label="Logit",hjust=-.2) +
  annotate(geom = "text", x="test", y=kapp.train[2], label="LDA",hjust=-.2) +
  annotate(geom = "text", x="test", y=kapp.train[3], label="QDA",hjust=-.2) +
  annotate(geom = "text", x="train", y=kapp.train[4], label="R. Forest",hjust=-.2) +
  annotate(geom = "text", x="test", y=kapp.train[5], label="XgBoost",hjust=-.2)
```
...
```{r}
sens.train <- cbind(glm.train$byClass[1],lda.train$byClass[1],qda.train$byClass[1],rf.cm.train$byClass[1],xgb.cm.train$byClass[1])
models <- c("Logit","LDA","QDA","R. Forest","XgBoost")
plot_sens <- data.frame(sens[1,],sens.train[1,],models)
colnames(plot_sens) <- c("sensitivity_test", "sensitivity_train","model")

ggplot(plot_sens,aes(x=model))+
  geom_point(aes(y=sensitivity_train),col="#5DADE2",size=4)+
  geom_point(aes(y=sensitivity_test),col="#BA4A00",size=4)+
  geom_segment(aes(x=model, xend=model,
               y=min(sensitivity_test), yend=max(sensitivity_test)),
               linetype="dashed",
               size=0.1) +
...
```{r}
spec.train <- cbind(glm.train$byClass[2],lda.train$byClass[2],qda.train$byClass[2],rf.cm.train$byClass[2],xgb.cm.train$byClass[2])
models <- c("Logit","LDA","QDA","R. Forest","XgBoost")
plot_spec <- data.frame(spec[1,],spec.train[1,],models)
colnames(plot_spec) <- c("specificity_test", "specificity_train","model")

ggplot(plot_spec,aes(x=model))+
  geom_point(aes(y=specificity_train),col="#5DADE2",size=4)+
  geom_point(aes(y=specificity_test),col="#BA4A00",size=4)+
  geom_segment(aes(x=model, xend=model,
               y=min(specificity_test), yend=max(specificity_test)),
               linetype="dashed",
               size=0.1) +
  labs(title = "Specificity", x= "Model", y="Specificity") +
  coord_flip()
...
I did some graphs comparing the test data with the predicted data, maybe that? 

- __ROC curve of our final model, or max our final two__

ROC/AUC of Random Forest and XgBoost.
```{r}

plot.roc(ifelse(df_test$target_customer=="is_target",1,0),ifelse(rf.pred=="is_target",1,0),percent=T,main="ROC, Random Forest Model", print.auc=TRUE)

```
```{r}

plot.roc(ifelse(df_test$target_customer=="is_target",1,0),ifelse(xgb.pred=="is_target",1,0),percent=T,main="ROC, XgBoost Model", print.auc=TRUE)
