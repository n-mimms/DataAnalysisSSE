...
#partition the data evenly along the target variable: 80% will be training, 20% testing (alt. 70-30)
...
#Train the model (DONT RUN. use parallel processing):
svm_churn <- train(
  is_churn ~ home_delivery + num_items_sold + flag_multiple + num_returns,
  data = churn_train,
  method ="svmLinear")   #using svm inear
...
Same as above, but we use multiple cores so it goes quicker:
Read about it [here](https://nceas.github.io/oss-lessons/parallel-computing-in-r/parallel-computing-in-r.html). 
